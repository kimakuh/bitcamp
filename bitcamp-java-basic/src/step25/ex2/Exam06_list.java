...
public class Exam06_list {
    ...
    public static void main(String[] args) throws Exception {
        ...
        PreparedStatement stmt = con.prepareStatement("select tno,titl,startd,endd,state,member,team from ex_task");

        // SQL을 실행할 때는 파라미터로 SQL문을 넘겨주지 않는다.
        ResultSet rs = stmt.executeQuery();
        while (rs.next()) {
            System.out.printf("%d,%s,%s,%s,%d,%s,%s\n", rs.getInt("tno"), rs.getString("titl"), rs.getDate("startd"),
                    rs.getDate("endd"), rs.getInt("state"),rs.getString("member"),rs.getString("team"));
        }
